...
function useMotionValueEvent(value, event, callback) {
  (0, import_react14.useInsertionEffect)(() => value.on(event, callback), [value, event, callback]);
}
...
function useScroll({ container, target, layoutEffect = true, ...options } = {}) {
  ...
  const useLifecycleEffect = layoutEffect ? useIsomorphicLayoutEffect : import_react15.useEffect;
  useLifecycleEffect(() => {
...
function useMotionValue(initial) {
  ...
  if (isStatic) {
    const [, setLatest] = (0, import_react16.useState)(initial);
    (0, import_react16.useEffect)(() => value.on("change", setLatest), []);
  }
  return value;
}
...
function useSpring(source, config = {}) {
  ...
  const activeSpringAnimation = (0, import_react17.useRef)(null);
  const value = useMotionValue(isMotionValue(source) ? toNumber(source.get()) : source);
  const latestValue = (0, import_react17.useRef)(value.get());
  const latestSetter = (0, import_react17.useRef)(() => {
  });
...
function useCycle(...items) {
  const index = (0, import_react24.useRef)(0);
  const [item, setItem] = (0, import_react24.useState)(items[index.current]);
  const runCycle = (0, import_react24.useCallback)(
    (next) => {
...
function useInstantTransition() {
  ...
  const unlockOnFrameRef = (0, import_react26.useRef)(-1);
